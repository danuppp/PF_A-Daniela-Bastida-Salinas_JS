document.addEventListener("DOMContentLoaded", function () {
  ...
  const apiKeyClima = '3021fcd354bbe034341e1102c05af125';

  const urlClima = `https://api.openweathermap.org/data/2.5/weather?q=${ciudad}&appid=${apiKeyClima}&units=metric`;

  obtenerInfoClima(urlClima);

  function obtenerInfoClima(url) {
    fetch(url)
      ...
      .then(data => {
        ...
        climaInfo.innerHTML = `Clima en ${ciudad}: ${temperatura}°C`;
      })
      .catch(error => {
